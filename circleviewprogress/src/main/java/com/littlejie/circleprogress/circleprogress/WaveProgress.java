...
public class WaveProgress extends View {
    ...
    private CharSequence mHint = "步".toString();
    private int mHintColor;
    private float mHintSize;
    //绘制步数
    private TextPaint stepPaint;
    private CharSequence mstepHint;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mHint = context.getResources().getString(R.string.steps);
    ...
    private void initPaint() {
        ...
        //绘制步数
        stepPaint = new TextPaint();
        stepPaint.setAntiAlias(antiAlias); //抗锯齿
        stepPaint.setTextSize(25);
        stepPaint.setColor(Color.WHITE);
        stepPaint.setTextAlign(Paint.Align.RIGHT);
    ...
    private void drawProgress(Canvas canvas) {
        ...
        //canvas.drawText(getResources().getString(R.string.steps), mCenterPoint.x+(mPercentPaint.measureText(String.format("%.f", mPercent * mMaxValue)))/2+30, y, mHintPaint);
        ...
        if (mHint != null) {
            float hys = mCenterPoint.y * 2 / 3 - (mHintPaint.descent() + mHintPaint.ascent()) / 2;
            canvas.drawText(mHint.toString(), mCenterPoint.x, hys, mHintPaint);
        }
//        Log.e(TAG,"---------mValue-="+mValue+"--=="+mPercent * mMaxValue+"---="+decimalFormat.format(mValue));
//        Log.e(TAG,"---长度="+(mCenterPoint.x+(mPercentPaint.measureText(String.format("%s", decimalFormat.format(mValue))))/2+30)+"---="+(mPercentPaint.measureText(String.format("%s", decimalFormat.format(mValue)))));
//        Log.e(TAG,"---长度="+(mCenterPoint.x+(mPercentPaint.measureText(String.format(Locale.US,"%s", decimalFormat.format(mValue))))/2+30)+"---="+((mPercentPaint.measureText(String.format(Locale.US,"%f", mValue)))/2+30)+"\n"+"--="+((mPercentPaint.measureText(String.format(Locale.US,"%s", decimalFormat.format(mValue))))/2+30));
    }
